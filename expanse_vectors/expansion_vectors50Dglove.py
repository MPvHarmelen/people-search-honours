...
find_words_neighbours('vectors_glove_klein.p', 'words_neighbours_klein_glove.p', True)

#find_words_neighbours('words_vectors_glove.p', 'words_neighbours_glove.p', True)

def expand(file_topics, folder, save_as, vector_file, k):
    with open(file_topics) as topics, open(folder + '/k='+str(k-1)+save_as, 'w') as expanded_topics, open(vector_file, 'rb') as neighbours_file:
...
def rotzooi(file_topics, folder, save_as, vector_file):
	expand(file_topics, folder, save_as, vector_file, 1)
	expand(file_topics, folder, save_as, vector_file, 2)
	expand(file_topics, folder, save_as, vector_file, 3)
	expand(file_topics, folder, save_as, vector_file, 5)
	expand(file_topics, folder, save_as, vector_file, 9)
	expand(file_topics, folder, save_as, vector_file, 17)


file_topics = '../../tu-expert-collection/topics/topics_nl.ldf'
save_as = 'nl_expanded_topics_klein_comp.ldf'
folder = 'topicsnl_klein_glove'
vector_file = 'words_neighbours_klein_glove.p'

rotzooi(file_topics, folder, save_as, vector_file)

file_topics = '../../tu-expert-collection/topics/topics_uk.ldf'
save_as = 'uk_expanded_topics_klein_comp.ldf'
folder = 'topicsuk_klein_glove'
