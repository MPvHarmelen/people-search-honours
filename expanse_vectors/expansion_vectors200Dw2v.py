...
find_words_neighbours('vectors_comp_groot.bin', 'words_neighbours_groot_comp.p', False)


#find_words_neighbours('words_vectors_glove.p', 'words_neighbours_glove.p', True)

def expand(file_topics, folder, save_as, k):
    with open(file_topics) as topics, open(folder + '/k='+str(k-1)+save_as, 'w') as expanded_topics, open('words_neighbours_groot_comp.p', 'rb') as neighbours_file:
...
save_as = 'nl_expanded_topics_groot_comp.ldf'
folder = 'topicsnl_groot'

rotzooi(file_topics, folder, save_as)

file_topics = '../../tu-expert-collection/topics/topics_uk.ldf'
save_as = 'uk_expanded_topics_groot_comp.ldf'
folder = 'topicsuk_groot'
